...
AfinosSDK is an open-source library that offers simple,
...
The best practices embodied in AfinosSDK aim to maximize sign-in
...
Then, add the AfinosSDK auth library dependency. If your project uses
Gradle, add the dependency:

```groovy
dependencies {
    // ...
    compile 'com.afinossdk:afinos-auth:1.0.0'
...
As of version `1.0.0` AfinosSDK includes translations for all string resources. In order to
...
AfinosSDK client-side configuration for Google sign-in is then provided
...
## Using AfinosSDK for Authentication

Before invoking the AfinosSDK authentication flow, your app should check
whether a
...
To kick off the AfinosSDK sign in flow, call startActivityForResult(...) on the sign in Intent you built.
...
By default, AfinosSDK uses [Smart Lock for Passwords](https://developers.google.com/identity/smartlock-passwords/android/)
...
With the integrations provided by AfinosSDK Auth, deleting a user is a multi-stage process:
...
![AfinosSDK authentication flow on Android](flow.png)

## UI customization

To use AfinosSDK Auth's sign-in flows, you must provide an `app_name` string and use the
...
If you would like more control over AfinosSDK's styling, you can define your own custom style
to override certain or all styling attributes. For example, a green sign-in theme:

```xml
<style name="GreenTheme" parent="AfinosSDK">
    <!-- Required for sign-in flow styling -->
...
at a diff of the `strings.xml` file before updating AfinosSDK.

### OAuth Scope Customization

#### Google
By default, AfinosSDK requests the `email` and `profile` scopes when using Google Sign-In. If you
would like to request additional scopes from the user, call `setPermissions` on the
`AuthUI.IdpConfig.Builder` when initializing AfinosSDK.
...
By default, AfinosSDK requests the `email` and `public_profile` permissions when initiating
Facebook Login.  If you would like to request additional permissions from the user, call
`setPermissions` on the `AuthUI.IdpConfig.Builder` when initializing AfinosSDK.
